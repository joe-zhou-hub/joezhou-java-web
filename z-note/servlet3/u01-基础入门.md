...
- 请求到来时：
    ...
    - web容器为该请求分配一个线程，该线程负责调用servlet类的 `service()`。
    - web容器将 `req` 和 `resp` 对象传递给 `service()`。
    - `service()` 根据请求头决定调用 `doGet()` 或者 `doPost()`，并将 `req` 和 `resp` 传递给它。
    - `doGet()` 或 `doPost()` 开始执行代码并做出响应。
    - 线程结束，线程销毁或者被遣返回线程池，`req` 和 `resp` 出作用域，等待GC。
    - 客户得到最终响应。
- 关闭服务器时：
    - web容器调用servlet的 `destroy()` 做一些清理工作，只执行一次。

> 如果配置 `<servlet></servlet>` 中添加了`<load-on-startup>1</load-on-startup>`，则 `init()` 方法伴随着servlet的实例化而执行，里面的值只要大于等于0即可，正数值越小加载优先级越高，如果小于0或者不配置，则会在第一次servlet请求的时候执行，且只执行一次。
