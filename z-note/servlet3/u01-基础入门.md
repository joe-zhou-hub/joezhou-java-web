...
- 继承：开发 `XmlServlet.java` 继承 `HttpServlet`：
...
- 开发：在 `doGet()` 中开发即时插入系统时间代码：
    ...
    - `resp.getWriter()`：获取响应字符流，调用 `println()` 回写数据。
    - `resp.getOutputStream()`：获取响应字节流，调用 `println()` 回写数据。
- 测试：`http://localhost:8080/servlet3/servlet/xmlServlet`。
