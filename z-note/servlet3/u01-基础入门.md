...
- XML配通情况下，可以在 `<servlet>` 中使用 `<init-param>` 子标签完成初始化参数设置：
    - `<param-name>tel</param-name>`
    - `<param-value>182</param-value>` 
- 注解配通情况下，可以在 `@WebServlet` 中使用 `initParams` 属性完成初始化参数设置：
    - `initParams = { @WebInitParam(name="tel", value="182")... }`。
- 获取初始化参数：可以在 `doGet()` 中获取：
    - `super.getServletConfig().getInitParameter(KEY)`：获取当前servlet的初始化参数。
    - `ServletConfig` 对象不存在 `setInitParameter()`。 

**源码：** /servlet3/..ServletConfigXmlServlet.java

**配置：** /servlet3/..web.xml

**源码：** /servlet3/..ServletConfigAnnotationServlet.java

## 2.4 全局参数

**概念：** `<context-param>` 可以在servlet在初始化时设置一些共享的String类型参数：
- 在 `web.xml` 中添加 `<context-param>` 子标签完成全局参数设置：
    - `<param-name>location</param-name>`
    - `<param-value>china</param-value>` 
- 获取全局参数：可以在 `doGet()` 中获取：
    - `super.getServletContext().getInitParameter(KEY)`：获取全局参数。
    - `ServletContext` 对象不存在 `setInitParameter()`。 
