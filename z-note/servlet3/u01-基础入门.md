...
- 测试：`http://localhost:8080/servlet3/servlet/xml`。
...
- 开发 `AnnoServlet.java`，完成继承重写步骤。
    - 可以使用IDEA直接创建servlet类型的java类。
- 对servlet类添加 `@WebServlet` 注解：
    - `@WebServlet("/s/anno")`
    - `@WebServlet(value = "/s/anoo")`：value可以省略
