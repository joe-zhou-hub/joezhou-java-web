...
- 开发 `XmlServlet.java` 继承 `HttpServlet`：
    - 还可以选择让 `XmlServlet.java` 实现 `Servlet` 接口，已过时。
    - 还可以选择让 `XmlServlet.java` 继承 `GenericServlet` 接口，已过时。
...
- 测试：`{{tomcat}}/servlet3/servlet/xml`。
...
- 开发 `AnnotationServlet.java`，完成继承重写步骤。
...
- 可以通过 `@WebServlet` 的 `initParams` 属性设置多组：
    - `initParams={@WebInitParam(name="tel", value="18210")}`。
