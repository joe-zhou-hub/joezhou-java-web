...
- `HttpSessionActivationListener`：监听 `HttpSession` 的迁移动作：
    - `sessionDidActivate()`：会话在一个vm上钝化时触发。
    - `sessionWillPassivate()`：会话在另一个vm上激活时触发。
- `ServletContextListener`：监听 `ServletContext` 的生死：
    - `contextInitialized()`：服务出生时触发。
    - `contextDestroyed()`：服务死亡时触发。
- `ServletContextAttributeListener`：监听 `ServletContext` 域中属性：
    - `attributeAdded()`：域中属性被添加时触发。
    - `attributeRemoved()`：域中属性被移除时触发。
    - `attributeReplaced()`：域中属性被修改时触发。
