...
- 重写过滤器类的 `contextInitialized()/contextDestroyed()`：初始化/销毁方法。
...
**需求：**
1. 服务器启动的时候，创建一个变量作为计数器，服务器不关闭，这个变量就要一直存在。
2. 服务器关闭的时候，销毁这个计数器变量。
3. 服务器启动状态下，只要有人成功登录，计数器加1，并打印："当前第x个人登录了您的网站！"

**源码：** /servlet3/
- src: `c.j.listener.VisitorCountListener`
- src: `c.j.listener.VisitorServlet`
- psm: `{{tomcat}}/servlet3/api/visitor?meta=login&name=admin`

# 2. 案例 - 阻止重复登录

**源码：** MyRepeatLoginListener.java
```java
/**
 * @author JoeZhou
 */
