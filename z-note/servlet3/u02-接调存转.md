...
- `setSerializationInclusion(JsonInclude.Include incl)`：设置序列化过程的参与者：
    - `JsonInclude.Include.NON_NULL`：null值不参与序列化。
- `configure(SerializationFeature f, boolean state)`：设置序列化某些配置的开启或禁止：
    - `SerializationFeature.WRITE_DATES_AS_TIMESTAMPS`：将Date类型数据序列化成时间戳。
...
- `GsonBuilder()`：构造 `GsonBuilder` 构造器对象。
    ...
    - `setDateFormat("yyyy-MM-dd")`：设置Date数据格式。
    - `create()`：通过 `Gson` 构造器构造 `Gson` 核心对象。
- `toJson(Object obj)`：将数据序列化为JSON字符串并返回。
    - `Gson` 会自动将数字 `1/0` 序列化成 `true/false`。
